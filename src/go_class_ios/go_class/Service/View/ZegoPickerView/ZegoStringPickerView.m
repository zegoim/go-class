...
@interface ZegoStringPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@end
@implementation ZegoStringPickerView
...
+ (void)showStringPickerWithTitle:(NSString *)title
                  ...
                  defaultSelValue:(id)defaultSelValue
                     isAutoSelect:(BOOL)isAutoSelect
                          manager:(ZegoPickerViewManager *)manager
                      resultBlock:(CGXStringResultBlock)resultBlock
{
    if (dataSource == nil || dataSource.count == 0) {
        return;
    }
    ZegoStringPickerView *strPickerView = [[ZegoStringPickerView alloc] initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect manager:manager resultBlock:resultBlock];
...
{
    NSArray *dataSource = [ZegoStringPickerView showStringPickerDataSourceStyle:style Manager:manager];
...
+ (NSArray *)showStringPickerDataSourceStyle:(NSInteger)style Manager:(ZegoPickerViewManager *)manager
{
    ...
    switch (style) {
        ...
        case 7:
        {
            NSArray *heightAry =[ZegoStringPickerView ageArray:[dataSourceaa objectForKey:@"age"]];
        ...
        case 9:
        {
            NSArray *heightAry =[ZegoStringPickerView heightArray:[dataSourceaa objectForKey:@"height"]];
            dataSource = heightAry;
        }
            break;
        case 10:
        ...
        case 11:
        {
            NSArray *heightAry =[ZegoStringPickerView weightArray:[dataSourceaa objectForKey:@"weight"]];
            dataSource = heightAry;
        }
            break;
        case 12:
...
- (void)loadData {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (idx == 0) {
            ...
            // 判断数据源数组的第一个元素是什么类型
            if ([obj isKindOfClass:[NSArray class]]) {
                self.isSingleColumn = NO; // 非单列
            } else if ([obj isKindOfClass:[NSString class]]) {
                self.isSingleColumn = YES; // 单列
            } else {
                self.isDataSourceValid = NO; // 数组不合法
                return;
            }
        } else {
            // 判断数组的元素类型是否相同
    ...
    if (!_pickerView) {
        ...
        if (self.isSingleColumn) {
            [_dataSource enumerateObjectsUsingBlock:^(NSString *rowItem, NSUInteger rowIdx, BOOL *stop) {
                if ([self.selectedItem isEqualToString:rowItem]) {
                    [self.pickerView selectRow:rowIdx inComponent:0 animated:NO];
                    *stop = YES;
                }
            }];
        } else {
            [self.selectedItems enumerateObjectsUsingBlock:^(NSString *selectedItem, NSUInteger component, BOOL *stop) {
                [_dataSource[component] enumerateObjectsUsingBlock:^(id rowItem, NSUInteger rowIdx, BOOL *stop) {
                    if ([selectedItem isEqualToString:rowItem]) {
                        [self.pickerView selectRow:rowIdx inComponent:component animated:NO];
                        *stop = YES;
                    }
                }];
            }];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    for(UIView *singleLine in pickerView.subviews)
    {
        ...
        {
            singleLine.backgroundColor = self.manager.lineViewColor;
        }
    }
