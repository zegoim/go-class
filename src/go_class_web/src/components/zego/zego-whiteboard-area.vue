...
<template>
  <div class="zego-whiteboard-area" v-if="!!client">
    <slot></slot>
    <file-upload-percent-dialog
      v-if="fileUploadStatus === 1 || fileUploadStatus === 8"
...
export default {
  ...
  data() {
    return {
      ...
      activeViewIsH5: null, // 当前激活文件白板是否h5
  ...
  methods: {
    ...
    handleRemoteWhiteboardChange(res) {
      ...
      if (type === '1001' && data !== this.activeWBId && data) {
        ...
        if (!this.originWBViewList.length) {
          console.warn(
            'handleRemoteWhiteboardChange 列表长度不为0',
            this.originWBViewList.length,
    ...
    async initWhiteboardArea() {
      ...
      this.docsClient.on('onUpload', async (res) => {
        ...
        if (res.status === 1 && res.uploadPercent !== 100) {
          console.log(
            `文件${ZegoDocsViewUploadState[res.status]}，进度${res.uploadPercent}% :`,
            res
          )
    ...
    async createDocView(id) {
      ...
      if (this.activeViewIsPPTH5) this.stopPlay()
      if (this.activeViewIsH5) this.stopPlay()
    ...
    updateActiveView(activeWBView, flag = 1) {
      setTimeout(() => {
        ...
        this.activeViewIsH5 = !!(fileInfo && fileInfo.fileType === 4096)
    ...
    checkWBCreateTime(arr) {
      ...
      let peon = arr.reduce((cur, next) => {
        ...
        obj[next.getFileInfo()?.fileID || next.getID()]
          ? ''
          : (obj[next.getFileInfo()?.fileID || next.getID()] = true && cur.push(next))
